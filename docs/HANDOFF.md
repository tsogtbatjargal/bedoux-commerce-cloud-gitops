...
- The owner confirmed `cloud.bedoux.com`; accepted ADR 0021 supersedes ADR 0020 and records the
  owned-subdomain path required by ADR 0014. Website implementation has not started.
- The local implementation adds a disabled-by-default `route53-acm` Terraform module, exact
  `cloud.bedoux.com` profile, root outputs, CI profile validation, and
  `docs/runbooks/p12-1-domain-tls-session.md`.
- The module deliberately excludes the parent domain and registration. It stages the live work:
...
- T-1201 is not complete. Public DNS currently has a non-Route 53 parent delegation and
  ...
  add only the child-zone NS delegation.
...
2. Before opening a live session, owner confirms access to add the `cloud` NS delegation in the
   existing parent DNS and approval to retain the USD 0.50/month Route 53 child zone.
